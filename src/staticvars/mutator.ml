...
module Ivmutator : Statemutator with type state = resolution * int gvar_i list * FunctionResolution.t = struct
  ...
  let cfor (v : 'len gvar_i) ((_, s, e) : 'len grange) (loc : L.i_loc) (_ : state) ((for_res, for_statics, fres) : state)
      : state * annotation =
      let sres, eres = (expr_resolution s, expr_resolution e) in
      let range_res = min_resolution sres eres in
      if range_res < for_res then
        rs_iverror ~loc:loc.base_loc (ForStaticWithDynamicRange (for_statics, expr_dynamic_vars s @ expr_dynamic_vars e))
      else if gvar_resolution v < for_res then
        rs_iverror ~loc:loc.base_loc (ForStaticWithDynamicVariable (v, expr_dynamic_vars s @ expr_dynamic_vars e))
      else
        let for_statics =
            match gvar_resolution v with
            | Dynamic -> for_statics
            | Static ->
                v :: for_statics (*We add it here because it could be conflicting if added while checking declaration*)
        in
        ((for_res, for_statics, fres), ())

  let cfor_decl (v : int gvar_i) ((_, s, e) : 'len grange) (loc : L.i_loc) ((for_res, for_statics, fres) : state) :
      state =
      let sres, eres = (expr_resolution s, expr_resolution e) in
      let range_res = min_resolution sres eres in
  ...
  let cwhile
      ...
      if condition_res < while_res then
        rs_iverror ~loc:loc.base_loc (WhileStaticWithDynamicCondition (while_statics, expr_dynamic_vars condition))
      else
        ((while_res, while_statics, fres), ())

  let assign_logic (lv : int glval) (expr : 'len gexpr) (loc : L.i_loc) ((resolution, statics, fres) : state) :
      state * annotation =
  ...
  let cassign (lv : int glval) (_ : E.assgn_tag) (_ : 'len gty) (expr : 'len gexpr) (loc : L.i_loc) (state : state) :
      state * annotation =
      assign_logic lv expr loc state

  let fcall (lvs : int glvals) (fn : funname) (exprs : int gexprs) (loc : L.i_loc) ((res, statics, fres) : state) :
      state * annotation =
  ...
  let copn (lvs : int glvals) (_ : E.assgn_tag) (_ : 'asm Sopn.sopn) (exprs : int gexprs) (loc : L.i_loc) (state : state)
      : state * annotation =
      let zip = List.combine lvs exprs in
      List.fold_left (fun ((s, _) : state * annotation) (lv, expr) -> assign_logic lv expr loc s) (state, ()) zip
end
