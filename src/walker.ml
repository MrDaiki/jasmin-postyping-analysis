...
module type Statemutator = sig
  ...
  val cfor : ('len gvar_i) -> ('len grange) -> L.i_loc -> state -> state -> state * annotation
  val cfor_decl : int gvar_i -> ('len grange) -> L.i_loc -> state -> state
  val cwhile : E.align -> ('len gexpr) -> L.i_loc -> state -> state -> state * annotation
  val cassign : (int glval) -> E.assgn_tag -> ('len gty) -> ('len gexpr) -> L.i_loc -> state -> state * annotation
  val fcall : (int glvals) -> funname -> (int gexprs) -> L.i_loc -> state -> state * annotation
  val syscall : (int glvals) -> BinNums.positive Syscall_t.syscall_t -> ('len gexprs) -> L.i_loc -> state -> state * annotation
...
module TreeWalker (Mutator : Statemutator) = struct
  ...
  let rec walk_instr (state:Mutator.state) (instr: ('info,'asm) instr) : Mutator.state * (int, Mutator.annotation, 'asm) ginstr = 
  ...
  and walk_instr_r (instr: ('len,'info,'asm) ginstr_r) (loc:L.i_loc) (state : Mutator.state) : (int, Mutator.annotation, 'asm) ginstr_r * Mutator.state * Mutator.annotation = 
    ...
    | Cfor (x, gr, body) -> 
      walk_for x gr body loc state
    | Cwhile (al, body1, e, body2) -> 
      walk_while al body1 e body2 loc state
