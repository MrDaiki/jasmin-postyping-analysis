...
module ForwardAnalyser = struct
  ...
  module Make (Logic : ForwardAnalyserLogic) : S with type domain = Logic.domain = struct
    ...
    let analyse_assign (loc : Location.i_loc) (lv : lval) tag ty (expr : expr) (annotation : annot)
        =
        let annotation = bind_annotation annotation (Logic.assign loc lv tag ty expr) in
    ...
    *)
        ...
        let rec loop in_annotation =
            ...
            let _, loop_annotation =
                analyse_assign loc (Lvar proxy_var) AT_none (Location.unloc variable).v_ty
                  ...
                  loop_annotation
            in
            if included_annotation loop_annotation in_annotation Logic.included then
              (Cfor (variable, range, body), out_annotation)
            else
              loop (merge_annotation loop_annotation in_annotation Logic.merge)
        in
        let body, annotation = loop annotation in
        let out_annotation = bind_annotation annotation (Logic.forget proxy_var) in
    ...
    and analyse_while
        ...
        let rec loop (in_annotation : annot) =
            ...
            if included_annotation annotation_s2 in_annotation Logic.included then
              (Cwhile (al, b1, cond, (a, annotation_s1), b2), result_annotation)
            else
              loop (merge_annotation annotation_s2 in_annotation Logic.merge)
        in
    ...
    and analyse_instr_r
        ...
        | Copn (lvs, tag, sopn, es) ->
            let annotation = bind_annotation annotation (Logic.opn loc lvs tag sopn es) in
            (Copn (lvs, tag, sopn, es), annotation)
        | Ccall (lvs, fn, es) ->
            let annotation = bind_annotation annotation (Logic.funcall loc lvs fn es) in
            (Ccall (lvs, fn, es), annotation)
        | Csyscall (lvs, sc, es) ->
            let annotation = bind_annotation annotation (Logic.syscall loc lvs sc es) in
        ...
        | Cif (expr, th, el) ->
            ...
            (Cif (expr, th, el), merge_annotation annotation_th annotation_el Logic.merge)
