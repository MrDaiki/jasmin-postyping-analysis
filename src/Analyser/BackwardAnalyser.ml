(* open Jasmin
   ...
   module type BackwardAnalyserLogic = sig
     ...
     *)
     ...
     val account_positive : int gexpr -> domain -> domain

     val account_negative : int gexpr -> domain -> domain
   ...
   module BackwardAnalyser = struct
     ...
     struct
       ...
       let rec analyse_for (loc : Location.i_loc) variable (range : int grange) body in_domain :
           ...
           let _, domain =
               analyse_assign loc (Lvar proxy_var) AT_none (Location.unloc variable).v_ty
                 (GR.last range) in_domain
           in
           let condition = build_for_condition_expr proxy_var range in
           let domain = L.account_negative condition in_domain in
           let rec loop prev =
       ...
       and analyse_while
           ...
           (domain : domain) =
           let initial_domain = L.account_negative cond domain in
           let rec loop (cond_out_domain : domain) =
               let b1, domain_b1 = analyse_stmt b1 cond_out_domain in
               let b2, domain_b2 = analyse_stmt b2 domain_b1 in
               let domain = L.account_positive cond domain_b2 in
               if L.included cond_out_domain domain then
                 (Cwhile (al, b1, cond, (a, L.to_annotation cond_out_domain domain_b2), b2), domain_b1)
               else
       ...
       and analyse_instr_r
           ...
           | Cif (expr, th, el) ->
               ...
               let domain_th = L.account_positive expr domain_th in
               let domain_el = L.account_negative expr domain_el in
