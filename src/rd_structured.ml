...
module Rd = struct
  ...
  let cmp (rd1 : t) (rd2 : t) : bool = cmp_rd rd1.rd_out rd2.rd_out && cmp_rd rd1.rd_in rd2.rd_in
end
...
let repr_rd_var_instr (instr : int) (vars : (uid, int Set.t) Map.t) : unit =
    Printf.printf "i %d: \n" instr ;
    Map.iter (fun k v -> repr_rd_var k v) vars ;
    Printf.printf "\n"

let repr_rd (rd : Rd.t) : unit = Map.iter (fun k v -> repr_rd_var_instr k v) rd.rd_out
